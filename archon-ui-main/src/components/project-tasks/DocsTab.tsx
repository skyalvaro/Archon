...
export const DocsTab = ({
  ...
  const [selectedDocument, setSelectedDocument] = useState<ProjectDoc | null>(
    null,
  );
  ...
  const [viewMode, setViewMode] = useState<"beautiful" | "markdown">(
    "beautiful",
  );
  ...
  const [sourceType, setSourceType] = useState<"technical" | "business">(
    "technical",
  );
  ...
  const loadProjectDocuments = async () => {
    ...
    try {
      ...
      console.log(
        `Loaded ${projectDocuments.length} documents from project data`,
      );
    } catch (error) {
  ...
  const createDocumentFromTemplate = async (templateKey: string) => {
    ...
    const template =
      DOCUMENT_TEMPLATES[templateKey as keyof typeof DOCUMENT_TEMPLATES];
    if (!template) return;
  ...
  const loadProjectData = async () => {
    ...
    try {
      ...
      const technicalSourceIds = (projectData.technical_sources || []).map(
        (source: any) => source.source_id,
      );
      const businessSourceIds = (projectData.business_sources || []).map(
        (source: any) => source.source_id,
      );
  ...
  const technicalSources = transformToLegacyFormat(
    knowledgeItems.filter(
      (item) => item.metadata.knowledge_type === "technical",
    ),
  );

  const businessSources = transformToLegacyFormat(
    knowledgeItems.filter(
      (item) => item.metadata.knowledge_type === "business",
    ),
  );

  const toggleTechnicalSource = (id: string) => {
    setSelectedTechnicalSources((prev) =>
      prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id],
    );
  };
  const toggleBusinessSource = (id: string) => {
    setSelectedBusinessSources((prev) =>
      prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id],
    );
  };
      ...
      <div className="max-w-6xl pl-8">
            ...
            <div className="mb-8">
              <div className="p-6 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div className="text-gray-500 dark:text-gray-400">
                  Beautiful view mode temporarily unavailable
                </div>
              </div>
            </div>
          ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
          <KnowledgeSection
            ...
            color="blue"
            sources={selectedTechnicalSources.map((id) =>
          ...
          <KnowledgeSection
            ...
            color="purple"
            sources={selectedBusinessSources.map((id) =>
...
const TemplateModal: React.FC<{
  ...
  const getTemplateDescription = (key: string, template: any) => {
    const descriptions: Record<string, string> = {
      markdown_doc:
        "Simple markdown document for general documentation and notes.",
      meeting_notes:
  ...
  return (
    <div className="fixed inset-0 bg-gray-500/50 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
      <div
        className="relative p-6 rounded-md backdrop-blur-md w-full max-w-2xl
...
const KnowledgeSection: React.FC<{
  ...
  sources: any[];
  onAddClick: () => void;
}> = ({ title, color, sources = [], onAddClick }) => {
  const colorMap = {
    blue: {
      bg: "bg-blue-500/10",
    ...
    <section>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold text-gray-800 dark:text-white flex items-center">
          <span
            className={`w-2 h-2 rounded-full bg-${color}-400 shadow-[0_0_8px_rgba(59,130,246,0.6)] mr-2`}
          />
          {title}
        </h3>
        <button
          onClick={onAddClick}
          className={`px-3 py-1.5 rounded-md ${colorMap[color].buttonBg} ${colorMap[color].buttonHover} border ${colorMap[color].buttonBorder} ${colorMap[color].text} ${colorMap[color].buttonShadow} transition-all duration-300 flex items-center gap-2`}
        >
      ...
      <div
        className={`bg-white/10 dark:bg-black/30 border ${colorMap[color].border} rounded-lg p-4 backdrop-blur-sm relative overflow-hidden min-h-[200px]`}
      >
            ...
            {sources.map(
              (source) =>
                source && (
                  <div
                    key={source.id}
                    className="flex items-center gap-3 p-2 rounded-md bg-white/10 dark:bg-black/30 border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 transition-all"
                  >
                    {source.type === "url" ? (
...
const SourceSelectionModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-gray-500/50 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
      <div
        className="relative p-6 rounded-md backdrop-blur-md w-full max-w-3xl
          bg-gradient-to-b from-white/80 to-white/60 dark:from-white/10 dark:to-black/30
          border border-gray-200 dark:border-zinc-800/50
          shadow-[0_10px_30px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_10px_30px_-15px_rgba(0,0,0,0.7)]
          before:content-[''] before:absolute before:top-0 before:left-0 before:right-0 before:h-[2px]
          ...
          <div className="flex gap-3 mb-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-white/50 dark:bg-black/70 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white rounded-md py-2 pl-10 pr-3 focus:outline-none focus:border-blue-400 focus:shadow-[0_0_10px_rgba(59,130,246,0.2)] transition-all duration-300"
              />
            </div>
            <Button
              onClick={onAddSource}
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (method === "url") {
        ...
        showToast(
          (result as any).message || "Document uploaded successfully",
          "success",
        );
        onSuccess();
      }
    ...
    <div className="fixed inset-0 bg-gray-500/50 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
      <Card className="w-full max-w-2xl relative before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-[1px] before:bg-green-500">
        <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-6">
          Add {sourceType === "technical" ? "Technical" : "Business"} Knowledge
          Source
        </h2>
        ...
        <div className="flex gap-4 mb-6">
          <button
            ...
            className={`flex-1 p-4 rounded-md border ${method === "url" ? "border-blue-500 text-blue-600 dark:text-blue-500 bg-blue-50 dark:bg-blue-500/5" : "border-gray-200 dark:border-zinc-900 text-gray-500 dark:text-zinc-400 hover:border-blue-300 dark:hover:border-blue-500/30"} transition flex items-center justify-center gap-2`}
          >
        ...
        {method === "file" && (
          <div className="mb-6">
            <label
              htmlFor="file-upload"
              className="block text-gray-600 dark:text-zinc-400 text-sm mb-2"
            >
              Upload Document
            </label>
            <input
